...
info_path = os.path.dirname(os.getcwd()) + "/item_info/"

sql = "insert into {} (item_id,item_title,item_title_url,category_id,main_image_url," \
      "other_image_url,item_price,item_sales,item_scores,item_description,item_specifics," \
      "item_packaging,item_quantity,item_variations,attr_name,variations_image) value (\"{}\"," \
      "\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\"," \
      "\"{}\",\"{}\",\"{}\")"
...
class GetItemInfo(ApiRequest):
    def __init__(self,key):
        ...
        """
        self.key = key
        self.table_name = key.strip("_url")
        self.item_info_file = open(info_path+key.strip("_url")+"_data.txt","a")
        # self.item_info_file = open(key+"data.txt","a")
    ...
    def get_item_info_thread(self,url):
        ...
        if info_dic:
            ...
            insert_sql = sql.format(self.table_name, info_dic["item_id"], MySQLdb.escape_string(info_dic["item_title"]),
                                    MySQLdb.escape_string(info_dic["item_title_url"]),
                                    str(info_dic["category_id"]), MySQLdb.escape_string(info_dic["main_image_url"]),
                                    MySQLdb.escape_string(info_dic["other_image_url"]),
                                    str(info_dic["item_price"]), str(info_dic["item_sales"]),
                                    str(info_dic["item_scores"]), MySQLdb.escape_string(info_dic["item_description"]),
                                    MySQLdb.escape_string(info_dic["item_specifics"]),
                                    MySQLdb.escape_string(info_dic["item_packaging"]), str(info_dic["item_quantity"]),
                                    MySQLdb.escape_string(info_dic["item_variations"]),
                                    MySQLdb.escape_string(info_dic["attr_name"]),
                                    MySQLdb.escape_string(info_dic["variations_image"]))
            db.commit_record(insert_sql)
            self.lock.release()
        else:
            return
